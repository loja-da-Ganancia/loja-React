...
import { adicionarFavoritoGlobal, removerFavoritoGlobal, fetchFavoritosThunk } from "../slices/favoritosSlice";
...
export default function Marketplace() {
  ...
  // ====================================================
  // CARREGAR FAVORITOS NO F5 (RELOAD)
  // ====================================================
  useEffect(() => {
    if (currentUser) {
      dispatch(fetchFavoritosThunk());
    }
  }, [dispatch, currentUser]);

  const abrirAfiliada = (url, afiliada, idCarta) => {
    if (currentUser) {
      registrarClique(afiliada, idCarta);
    }
  ...
  function toggleFavorito(carta) {
    ...
    let isFavoritado = favoritos.some((f) => String(f.id || f._id) === String(carta.id));

    if (isFavoritado) {
      dispatch(removerFavoritoGlobal({ nome: carta.name, username: currentUser.username }));
      setToastMensagem("❌ Retirado dos favoritos!");
      ...
      dispatch(adicionarFavoritoGlobal(novaCarta));
      setToastMensagem("⭐ Adicionado aos favoritos!");
      setMostrarToast(true);
    }
  ...
  return (
    <main className="flex-grow-1">
          ...
          <div className="col-lg-9">
            <div className="row g-4">
              ...
              {!carregando && cartasDestaPagina.map((carta) => {
                ...
                let isFavoritado = Boolean(currentUser) && favoritos.some((f) => String(f.id || f._id) === String(carta.id));
                  ...
                  <div className="col-12 col-sm-6 col-md-4 mb-4" key={carta.id}>
                    <div className="card h-100 shadow-sm rounded-3">
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); toggleFavorito(carta); }}
                        style={{
                          background: 'transparent',
                          border: 'none',
                          fontSize: '1.5rem',
                          color: isFavoritado ? '#ffc107' : '#ffffff', 
                          outline: 'none'
                        }}
                      >
                        {isFavoritado ? '⭐' : '☆'}
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-7">
                        ...
                        <div className="vendor-list mb-4">
                          <div onClick={() => abrirAfiliada(`https://www.tcgplayer.com/search/yugioh/product?q=${encodeURIComponent(nomeInglesModal)}`, 'TCGPlayer', cartaSelecionada.id)} style={{ cursor: 'pointer' }}>
                          ...
                          <div onClick={() => abrirAfiliada(`https://www.amazon.com/s?k=${encodeURIComponent("Yu-Gi-Oh! " + nomeInglesModal)}&tag=${AMAZON_AFFILIATE_TAG}`, 'Amazon', cartaSelecionada.id)} style={{ cursor: 'pointer' }}>
                            <div className="vendor-card">
                          ...
                          <div onClick={() => abrirAfiliada(`https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(nomeInglesModal + " yugioh")}`, 'eBay', cartaSelecionada.id)} style={{ cursor: 'pointer' }}>
                            <div className="vendor-card">
                          ...
                          <div onClick={() => abrirAfiliada(`https://www.cardmarket.com/en/YuGiOh/Products/Search?searchString=${encodeURIComponent(nomeInglesModal)}`, 'Cardmarket', cartaSelecionada.id)} style={{ cursor: 'pointer' }}>
                            <div className="vendor-card">
      ...
      {mostrarModalLogin && (
        <>
          <div className="modal fade show d-block" tabIndex="-1" onClick={() => setMostrarModalLogin(false)} style={{zIndex: 1080}}>
            <div className="modal-dialog modal-dialog-centered" onClick={(e) => e.stopPropagation()}>
              <div className="modal-content text-center p-4 shadow-lg" style={{backgroundColor: '#161b22', border: '1px solid #30363d', borderRadius: '15px'}}>
                ...
                <h4 className="text-white fw-bold mb-3">Acesso Restrito</h4>
                <p style={{ color: '#8b949e', fontSize: '1.05rem', lineHeight: '1.5' }}>
                  É necessário fazer login na sua conta para conseguir favoritar as cartas e guardar a sua coleção!
                </p>
                <div className="d-flex justify-content-center gap-3 mt-4">
                  <button className="btn btn-outline-secondary fw-bold px-4" onClick={() => setMostrarModalLogin(false)}>
                    Cancelar
                  </button>
                  <button className="btn btn-info fw-bold text-dark px-4 shadow" onClick={() => navigate('/contas')}>
                    Fazer Login
                  </button>
                </div>
              </div>
      ...
      <div style={{
        ...
        background: toastMensagem.includes('Retirado') ? '#dc3545' : '#28a745', 
