...
import { removerFavoritoGlobal, fetchFavoritosThunk } from "../slices/favoritosSlice";
...
export default function Favoritos() {
  ...
  const meusFavoritos = useSelector((state) => state.favoritos.items); 
  
  const [modalInfo, setModalInfo] = useState(null);
  const [carregandoModal, setCarregandoModal] = useState(false);
  const [imagemZoom, setImagemZoom] = useState(false);
  
  // NOVO ESTADO: Controla o modal de exclusão estiloso
  const [cartaParaRemover, setCartaParaRemover] = useState(null);

  // ====================================================
  // DESPERTADOR DE CARREGAMENTO (BANCO DE DADOS)
  // ====================================================
  useEffect(() => {
    if (currentUser) {
      ...
      <div className="container mt-5 mb-5 d-flex flex-column align-items-center justify-content-center flex-grow-1" style={{ minHeight: '65vh' }}>
        <div className="text-center p-5 rounded-4 shadow-lg" style={{ backgroundColor: '#161b22', border: '1px solid #30363d', maxWidth: '500px' }}>
          ...
          <p className="mt-3 mb-4" style={{ color: '#8b949e', fontSize: '1.05rem', lineHeight: '1.6' }}>
            Você precisa estar conectado para aceder a esta área. Faça login ou crie uma conta gratuita para guardar os seus favoritos!
          </p>
  ...
  function solicitarRemocao(carta) {
    setCartaParaRemover(carta);
  }

  // Executa a remoção de fato se o usuário confirmar
  function confirmarRemocao() {
    if (cartaParaRemover) {
      dispatch(removerFavoritoGlobal({ nome: cartaParaRemover.nome, username: currentUser.username }));
      setCartaParaRemover(null);
    }
  }

  // Cancela e fecha o modal de exclusão
  ...
  async function abrirDetalhesFavorito(carta) {
    setModalInfo({
      nome: carta.nome, imagem: carta.imagem, tipo: "A carregar...", descricao: "A buscar informações ao banco de dados..."
    });
  ...
  return (
    <div className="container mt-4 mb-5 flex-grow-1">
      ...
      <div className="row g-4">
        {meusFavoritos.length === 0 ? (
          <div className="col-12 text-center mt-5" style={{color: '#8b949e', fontSize: '1.1rem'}}>
            Ainda não adicionou nenhuma carta aos favoritos. <br/>
            ...
            <div className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4" key={index}>
              <div className="card card-favorito h-100 shadow-sm rounded-3">
                ...
                <div className="card-footer bg-transparent border-0 pt-0 text-center pb-3">
                  {/* Atualizado para abrir o novo modal em vez do window.confirm */}
                  <button className="btn btn-outline-danger btn-sm w-100 fw-bold" onClick={() => solicitarRemocao(carta)}>
      ...
      {cartaParaRemover && (
        <>
          <div className="modal fade show d-block" tabIndex="-1" onClick={cancelarRemocao} style={{zIndex: 1080}}>
            <div className="modal-dialog modal-dialog-centered" onClick={(e) => e.stopPropagation()}>
              <div className="modal-content text-center p-4 shadow-lg" style={{backgroundColor: '#161b22', border: '1px solid #30363d', borderRadius: '15px'}}>
                ...
                <h4 className="text-white fw-bold mb-3">Remover Favorito?</h4>
                <p style={{ color: '#8b949e', fontSize: '1.05rem', lineHeight: '1.5' }}>
                  Tem certeza que deseja remover <br/>
                  <strong className="text-light">{cartaParaRemover.nome}</strong><br/>
                  da sua lista de favoritos?
                </p>
                <div className="d-flex justify-content-center gap-3 mt-4">
                  <button className="btn btn-outline-secondary fw-bold px-4" onClick={cancelarRemocao}>
                    Cancelar
                  </button>
                  <button className="btn btn-danger fw-bold px-4 shadow" onClick={confirmarRemocao}>
                    Sim, Remover
                  </button>
                </div>
              </div>
      ...
      {/* ==================================================== */}
      {/* MODAL DE DETALHES DA CARTA (EXISTENTE) */}
      {/* ==================================================== */}
      {modalInfo && (
        <>
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg" onClick={(e) => e.stopPropagation()}>
              <div className="modal-content" style={{backgroundColor: '#161b22', border: '1px solid #30363d'}}>
                ...
                <div className="modal-body text-center">
                  {carregandoModal && <p className="text-info mt-3">A carregar informações do banco de dados...</p>}
                  <div className="row">
