...
import { fetchPostsThunk, addPost, deletePost, editPost } from "../slices/postSlice";
import { fetchDecksThunk } from "../slices/decksSlice";
import { showToastAsync } from "../slices/uiSlice";
...
export default function Comunidade() {
  ...
  // ====================================================
  // DISPARO DE CARREGAMENTO (DESPERTADOR DA TELA)
  // ====================================================
  useEffect(() => {
  ...
  function confirmarRemocaoPost() {
    if (postParaRemover) {
      dispatch(deletePost(postParaRemover));
      setPostParaRemover(null);
    }
  }

  function cancelarRemocaoPost() {
    setPostParaRemover(null);
  }

  function abrirEdicaoPost(post) {
    setPostIdEmEdicao(post.id || post._id);
  ...
  function salvarEdicaoPost(e) {
    e.preventDefault();
    dispatch(editPost(postIdEmEdicao, { content: postContent }));
    fecharModais();
  }
  ...
  function copiarLink() {
    navigator.clipboard.writeText(window.location.href).then(() => { 
      dispatch(showToastAsync("🔗 Link da página copiado com sucesso!", "success")); 
      fecharModais(); 
    });
  }
  ...
  return (
    <main className="flex-grow-1 container mt-5 mb-5">
      <div className="d-flex justify-content-between align-items-center mb-4 border-bottom border-secondary pb-3 flex-wrap gap-3">
        ...
        <button className="btn btn-info fw-bold" onClick={abrirModalNovoPost}>Compartilhar Deck</button>
      </div>
                ...
                <div className="post-card p-4 mb-4 rounded-3 shadow-sm" key={currentPostId} style={{ backgroundColor: '#161b22', border: '1px solid #30363d' }}>
                  ...
                  <div className="d-flex justify-content-between mt-4 align-items-center flex-wrap gap-2">
                    <div>
                      {podeEditar && (
                        <>
                          <button className="btn btn-sm btn-outline-warning text-white fw-bold me-2" onClick={() => abrirEdicaoPost(post)}>Editar</button>
                          <button className="btn btn-sm btn-outline-danger fw-bold" onClick={() => solicitarRemocaoPost(currentPostId)}>Excluir</button>
                        </>
                      )}
                    </div>
                    <button className="btn btn-sm btn-info text-dark fw-bold px-4" onClick={() => abrirVisualizacaoDeckComunidade(deck.id || deck._id)}>Ver Deck</button>
                  </div>
                </div>
              );
            })
          ...
          <div className="modal fade show d-block" tabIndex="-1" onClick={cancelarRemocaoPost} style={{zIndex: 1080}}>
            <div className="modal-dialog modal-dialog-centered" onClick={(e) => e.stopPropagation()}>
              <div className="modal-content text-center p-4 shadow-lg" style={{backgroundColor: '#161b22', border: '1px solid #30363d', borderRadius: '15px'}}>
                ...
                <h4 className="text-white fw-bold mb-3">Excluir Publicação?</h4>
                <p style={{ color: '#8b949e', fontSize: '1.05rem', lineHeight: '1.5' }}>
                  Tem certeza que deseja excluir esta publicação? <br/>
                      ...
                      <select className="form-select bg-dark text-light border-secondary" required value={postDeckId} onChange={(e) => setPostDeckId(e.target.value)}>
                        {meusDecks.length === 0 ? <option value="">Crie um deck primeiro na aba Decks!</option> : (
                          <>
                            ...
                            {meusDecks.map((d) => <option value={d.id || d._id} key={d.id || d._id}>{d.nome}</option>)}
                          </>
                        )}
                      </select>
                    </div>
